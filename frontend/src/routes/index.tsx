...
import {
  ...
  UserPlus, MailCheck, Share2, Gamepad2, Award,
} from "lucide-react";
...
function ActivityCard({
  ...
  const descriptionHtml = (game.description ?? "")
    .replace(/<ul(?![^>]*class=)/gi, '<ul class="list-none space-y-3">')
    .replace(/<li>/gi, '<li class="flex items-start gap-3"><span class="text-white/90 mt-0.5 mr-3">✓</span><span>')
  ...
  return (
    <div className="relative overflow-hidden rounded-[2rem] min-h-[340px] shadow-elevated group">
      <img
        src={image}
        alt={game.title}
        width={1024}
      ...
      />
      <div
        className={`absolute inset-0 ${accent === "warm" ? "bg-gradient-to-t from-orange-950/90 via-orange-900/60 to-transparent" : "bg-gradient-to-t from-purple-900/95 via-purple-900/55 to-transparent"}`}
      />
      <div className="relative flex h-full flex-col justify-between p-7 text-white">
        <div className="flex items-start gap-4">
          <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-3xl bg-white/15 border border-white/20 shadow-lg backdrop-blur-lg">
            {iconImage ? (
          ...
          <div className="max-w-xl">
            ...
            {!hasHtml ? (
              <p className="mt-3 text-sm text-white/80 leading-relaxed">{summary}</p>
            ) : null}
          </div>
        </div>

        {hasHtml ? (
          <div className="mt-6 rounded-3xl border border-white/20 bg-white/10 p-5 backdrop-blur-lg prose prose-invert max-w-none">
            <div dangerouslySetInnerHTML={{ __html: descriptionHtml }} />
          </div>
        ) : bullets.length > 0 ? (
          <div className="mt-6 rounded-3xl border border-white/20 bg-white/10 p-5 backdrop-blur-lg">
            <ul className="space-y-3 text-sm text-white/90">
              {bullets.map((bullet, index) => (
                <li key={index} className="flex items-start gap-3">
                  <Check className="h-4 w-4 mt-0.5 text-white/90 shrink-0" />
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-white/70">
            Designed to keep teams moving, collaborating, and celebrating every moment.
          </p>
          <Link
            to="/create"
            search={{ activity: game.slug }}
            className="inline-flex items-center justify-center rounded-full border border-white/30 bg-white/10 px-5 py-2 text-sm font-semibold text-white transition hover:bg-white hover:text-foreground"
          >
            Explore Activity
          </Link>
        </div>
