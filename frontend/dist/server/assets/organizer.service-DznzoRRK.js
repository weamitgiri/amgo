import { a as apiClient, E as ENV } from "./router-CXCreoWL.js";
...
const organizerService = {
  ...
  getResults: () => apiClient.get(API_ENDPOINTS.organizer.results),
  /** Deactivate account (soft delete — billing/GST records retained) */
  deleteAccount: () => apiClient.post(API_ENDPOINTS.organizer.deleteAccount, {}),
  /** Payment history — every booking this organizer has been billed for */
  getInvoices: () => apiClient.get(API_ENDPOINTS.organizer.invoices),
  /**
   ...
   */
  downloadInvoice: async (bookingId, invoiceNo) => {
   ...
   */
  verifyPayment: (payload) => apiClient.post(API_ENDPOINTS.organizer.paymentVerify, payload),
  /** Reports a dismissed or failed checkout so the attempt is not left pending. */
  reportPaymentFailure: (payload) => apiClient.post(API_ENDPOINTS.organizer.paymentFailed, payload),
  /**
   * Recovery for a browser closed mid-payment — the webhook may have settled
   * the booking already.
   */
  getPaymentStatus: (orderId) => apiClient.get(API_ENDPOINTS.organizer.paymentStatus(orderId))
};
export {
