...
import { LogIn, Instagram, MessageCircle, Facebook, Linkedin, ArrowUp } from "lucide-react";
import { F as Flogo } from "./Flogo-CmHh6mL2.js";
import { useQuery } from "@tanstack/react-query";
import { a as apiClient } from "./router-CXCreoWL.js";
import { A as API_ENDPOINTS } from "./config-hU3VGSBW.js";
const NAV = [
  { label: "Overview", to: "/" },
...
function Header({ floating = false }) {
  ...
  return /* @__PURE__ */ jsx("header", { className: floating ? "absolute top-6 left-0 right-0 z-30 px-4" : "sticky top-4 z-30 px-4", children: /* @__PURE__ */ jsx("div", { className: "mx-auto max-w-6xl", children: /* @__PURE__ */ jsxs("div", { className: `grid grid-cols-[1fr_auto_1fr] items-center gap-4 rounded-[2rem] border px-4 py-3 backdrop-blur-xl ${floating ? "border-white/20 bg-white/10" : "border-border bg-card/80 shadow-card"}`, children: [
    /* @__PURE__ */ jsx("div", { className: "flex justify-start", children: /* @__PURE__ */ jsx(Link, { to: "/", children: /* @__PURE__ */ jsx(Flogo, { light: floating, width: 230, height: 65 }) }) }),
    /* @__PURE__ */ jsx("nav", { className: "hidden lg:flex items-center gap-8 justify-center", children: NAV.map((item) => {
      const active = item.to === location.pathname;
      return /* @__PURE__ */ jsx(
        "a",
        {
          href: item.to,
          className: `text-sm transition-colors ${floating ? "text-[#B7B7B7] hover:text-white" : "text-foreground/70 hover:text-foreground"} ${active ? floating ? "text-white font-medium" : "text-foreground font-medium" : ""}`,
          children: item.label
        },
        item.label
      );
    }) }),
    /* @__PURE__ */ jsx("div", { className: "flex justify-end", children: /* @__PURE__ */ jsxs(Link, { to: "/login", className: "group flex items-center gap-2 rounded-full border border-border bg-white pl-5 pr-1.5 py-1.5 text-[15px] font-medium text-foreground hover:bg-gray-50 transition-colors", children: [
      "Login",
      /* @__PURE__ */ jsx("span", { className: "grid h-9 w-9 place-items-center rounded-full bg-purple-100 text-[#8B5CF6] transition-transform group-hover:translate-x-0.5", children: /* @__PURE__ */ jsx(LogIn, { className: "h-[18px] w-[18px]", strokeWidth: 2.5 }) })
    ] }) })
  ] }) }) });
}
const publicService = {
   ...
   */
  getPaymentMethods: (amount) => apiClient.get(
    amount != null ? `${API_ENDPOINTS.public.paymentMethods}?amount=${encodeURIComponent(amount)}` : API_ENDPOINTS.public.paymentMethods,
    { auth: "none" }
  ),
...
function usePaymentMethods(amount) {
  return useQuery({
    queryKey: [...publicQueryKeys.paymentMethods, amount ?? null],
