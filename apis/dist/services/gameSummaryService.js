...
exports.buildGameSummaryPayload = buildGameSummaryPayload;
...
async function ensureParticipantRoleAssignment(groupId, gameId, participantId) {
    return (0, db_1.withTransaction)(async (conn) => {
        await conn.query('SELECT id FROM game_groups WHERE id = ? FOR UPDATE', [groupId]);
        const [participantRows] = (await conn.query(`SELECT id FROM game_participants
             WHERE group_id = ? AND email_verified_at IS NOT NULL
             ORDER BY COALESCE(email_verified_at, created_at) ASC`, [groupId]));
        const [roleRows] = (await conn.query('SELECT id FROM game_roles WHERE game_id = ? ORDER BY id ASC', [gameId]));
        const participants = participantRows;
        const roleIds = roleRows.map((r) => Number(r.id));
        if (participants.length === 0 || roleIds.length === 0)
            return null;
        const [sessionRows] = (await conn.query('SELECT id, participant_id, role_id FROM participant_sessions WHERE group_id = ?', [groupId]));
        ...
        for (const p of participants) {
            if (!sessionByParticipant.has(Number(p.id))) {
                const [inserted] = (await conn.query('INSERT INTO participant_sessions (group_id, participant_id, is_online) VALUES (?, ?, 0)', [groupId, p.id]));
        ...
        const allUnassigned = participants.length === roleIds.length &&
...
async function buildGameSummaryPayload(groupId, participantId) {
    const [groupRows] = await (0, db_1.query)(`SELECT gg.id, gg.status AS group_status, ob.game_id, ob.activity_id,
         ...
         WHERE gg.id = ?`, [groupId]);
    if (groupRows.length === 0 || !groupRows[0].game_row_id)
        return null;
    const row = groupRows[0];
    let participant = { id: 0, name: 'Participant' };
    if (participantId) {
        const [pRows] = await (0, db_1.query)('SELECT id, name FROM game_participants WHERE id = ? AND group_id = ?', [participantId, groupId]);
        if (pRows.length > 0) {
            participant = { id: Number(pRows[0].id), name: pRows[0].name || 'Participant' };
        }
    }
    const [roleRows] = await (0, db_1.query)(`SELECT id, role_type, character_name, subtitle, role_image, objective, what_you_know, keep_in_mind
         FROM game_roles WHERE game_id = ? ORDER BY id ASC`, [row.game_row_id]);
    const [photoRows] = await (0, db_1.query)('SELECT id, label, image FROM game_photos WHERE game_id = ? ORDER BY photo_number ASC', [row.game_row_id]);
    const [clueRows] = await (0, db_1.query)('SELECT id, clue_title, clue_short_description, clue_detail, clue_image FROM game_clues WHERE game_id = ? ORDER BY id ASC', [row.game_row_id]);
    const [ruleRows] = await (0, db_1.query)('SELECT id, rule_text, `order` FROM game_rules WHERE game_id = ? ORDER BY `order` ASC', [row.game_row_id]);
    const [cardRows] = await (0, db_1.query)(`SELECT suspect_label, tag, profile_text, why_suspicious, suggested_questions, appears_at_secs, closes_at_secs
         FROM investigator_cards WHERE game_id = ? ORDER BY card_number ASC`, [row.game_row_id]);
    let myRoleId = null;
    if (participant.id) {
        myRoleId = await ensureParticipantRoleAssignment(Number(row.id), Number(row.game_row_id), Number(participant.id));
    }
    ...
    if (row.group_status === 'active') {
        await (0, timerService_1.ensureCaseSummaryTimer)(Number(row.id), Number(row.case_summary_view_secs) || 300);
    }
    ...
    const [sessionRows] = await (0, db_1.query)(`SELECT ps.id, ps.role_id, ps.participant_id, gp.name AS participant_name
    ...
    let roleStrategyRows = [];
    if (myRoleId) {
        const [rows] = await (0, db_1.query)('SELECT card_number, heading, body_content FROM strategy_cards WHERE role_id = ? ORDER BY card_number ASC', [myRoleId]);
        roleStrategyRows = rows;
    }
    ...
    const roles = roleRows.map((r, index) => {
        ...
        const base = {
            ...
            name,
            short: isYou ? `${(0, pseudonym_1.shortName)(name, Number(r.id))} (You)` : (0, pseudonym_1.shortName)(name, Number(r.id)),
    ...
    const rules = ruleRows.map((r, index) => ({
        id: Number(r.id),
        title: `Game Rule ${index + 1}`,
        description: r.rule_text,
        details: [r.rule_text],
    }));
    const strategy_slides = cardRows.map((c) => ({
