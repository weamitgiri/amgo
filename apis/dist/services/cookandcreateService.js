...
exports.getCCTemplateById = getCCTemplateById;
exports.getInstanceById = getInstanceById;
exports.round2StepLetter = round2StepLetter;
exports.ensureRound2ReviewStartedAt = ensureRound2ReviewStartedAt;
exports.ensureRound2TurnState = ensureRound2TurnState;
exports.getRound2TurnParticipantId = getRound2TurnParticipantId;
exports.getRound2TurnCount = getRound2TurnCount;
exports.getMyRound2TurnIndex = getMyRound2TurnIndex;
exports.ensureCCTimer = ensureCCTimer;
exports.getOrCreateCCInstance = getOrCreateCCInstance;
exports.ensureCCParticipantSessions = ensureCCParticipantSessions;
...
exports.finalizeRound1 = finalizeRound1;
exports.checkRound1Completion = checkRound1Completion;
exports.saveRound2Step = saveRound2Step;
exports.saveRound2StepVote = saveRound2StepVote;
exports.advanceRound2ToReview = advanceRound2ToReview;
exports.advanceRound2Turn = advanceRound2Turn;
exports.finalizeRound2Review = finalizeRound2Review;
exports.checkRound2ReviewCompletion = checkRound2ReviewCompletion;
exports.saveRound3Message = saveRound3Message;
exports.saveRound3ImpostorVote = saveRound3ImpostorVote;
exports.startRound3DiscussionTimer = startRound3DiscussionTimer;
exports.advanceRound3ToVoting = advanceRound3ToVoting;
exports.respondToDoubleDown = respondToDoubleDown;
exports.finalizeRound3 = finalizeRound3;
exports.checkRound3VotingCompletion = checkRound3VotingCompletion;
exports.getCCRatingCategories = getCCRatingCategories;
exports.getCCRules = getCCRules;
exports.getOtherDishes = getOtherDishes;
 ...
 */
async function getCCIngredients(templateId) {
    const [rows] = await (0, db_1.query)(`SELECT i.id, i.name, i.image_url, i.is_absurd
         ...
         ORDER BY ti.\`order\` ASC`, [templateId]);
    return rows.map((r) => ({
        id: Number(r.id),
        name: r.name,
 ...
 */
async function getCCTemplateById(templateId) {
    const [rows] = await (0, db_1.query)(`SELECT * FROM cc_game_templates WHERE id = ? LIMIT 1`, [templateId]);
    if (rows.length === 0)
        return null;
    return mapTemplateRow(rows[0]);
}
function mapInstanceRow(r) {
    return {
        id: Number(r.id),
...
async function getInstanceById(instanceId) {
    const [rows] = await (0, db_1.query)(`SELECT * FROM cc_game_instances WHERE id = ? LIMIT 1`, [instanceId]);
    if (rows.length === 0)
        return null;
 ...
 */
async function getGroupParticipantIds(groupId) {
    const [rows] = await (0, db_1.query)('SELECT id FROM game_participants WHERE group_id = ?', [groupId]);
 ...
 */
async function getRound2TurnOrder(instanceId, groupId) {
    const [rows] = await (0, db_1.query)('SELECT round2_turn_order FROM cc_game_instances WHERE id = ? LIMIT 1', [
        instanceId,
    ]);
    ...
    const [fallback] = await (0, db_1.query)(`SELECT id FROM game_participants WHERE group_id = ?
         ORDER BY COALESCE(email_verified_at, created_at) ASC, id ASC`, [groupId]);
 ...
 */
async function assignRound2TurnOrder(instanceId, groupId) {
    const [rows] = await (0, db_1.query)(`SELECT id FROM game_participants WHERE group_id = ?
         ORDER BY COALESCE(email_verified_at, created_at) ASC, id ASC`, [groupId]);
    ...
    await (0, db_1.query)('UPDATE cc_game_instances SET round2_turn_order = ? WHERE id = ? AND round2_turn_order IS NULL', [
        JSON.stringify(ids),
        instanceId,
    ]);
}
/** Turn 0 -> 'A', turn 1 -> 'B', ... Group size is capped well under 26. */
function round2StepLetter(turnIndex) {
 ...
 */
async function ensureRound2ReviewStartedAt(instanceId, groupId) {
    ...
    if (instance.status !== 'round2' || instance.round2_phase !== 'review')
        return;
    if (instance.round2_review_started_at)
        return;
    const [timerRows] = await (0, db_1.query)(`SELECT expires_at FROM timers
    ...
    if (!expiresAt)
        return;
    await (0, db_1.query)(`UPDATE cc_game_instances
         SET round2_review_started_at = DATE_SUB(?, INTERVAL ? SECOND)
         WHERE id = ? AND round2_review_started_at IS NULL`, [expiresAt, template.round2_review_timer_secs, instanceId]);
}
/**
 ...
 */
async function ensureRound2TurnState(instanceId, groupId) {
    const data = await getInstanceById(instanceId);
    if (!data)
        return;
    const { instance, template } = data;
    if (instance.status !== 'round2' || instance.round2_phase !== 'submit')
        return;
    if (instance.round2_turn_index !== null)
        return;
    const [stepRows] = await (0, db_1.query)('SELECT participant_id, step_letter FROM cc_round2_steps WHERE instance_id = ? ORDER BY step_letter ASC', [instanceId]);
    const alreadySubmitted = (stepRows || []).map((r) => Number(r.participant_id));
    const [participantRows] = await (0, db_1.query)(`SELECT id FROM game_participants WHERE group_id = ?
         ORDER BY COALESCE(email_verified_at, created_at) ASC, id ASC`, [groupId]);
    const remaining = (participantRows || [])
        .map((r) => Number(r.id))
        .filter((id) => !alreadySubmitted.includes(id));
    ...
    const [claimHeader] = await (0, db_1.query)(`UPDATE cc_game_instances SET round2_turn_order = ?, round2_turn_index = ?, round2_turn_started_at = NOW(), updated_at = NOW()
         WHERE id = ? AND round2_turn_index IS NULL`, [JSON.stringify(order), resumeAt, instanceId]);
    ...
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round2_turn_changed', { turn_index: resumeAt });
    await ensureCCTimer(groupId, instanceId, 'cc_round2_turn', template.round2_submit_timer_secs);
}
async function getRound2TurnParticipantId(instanceId, groupId, turnIndex) {
    if (turnIndex === null || turnIndex < 0)
        return null;
    const order = await getRound2TurnOrder(instanceId, groupId);
    return order[turnIndex] ?? null;
}
async function getRound2TurnCount(instanceId, groupId) {
...
async function getMyRound2TurnIndex(instanceId, groupId, participantId) {
 ...
 */
async function ensureCCTimer(groupId, instanceId, timerType, secs) {
    try {
        await (0, db_1.query)(`UPDATE timers SET is_active = 0 WHERE group_id = ? AND timer_type IN (?) AND is_active = 1`, [groupId, CC_TIMER_TYPES]);
        ...
        await (0, db_1.query)('INSERT INTO timers (group_id, timer_type, reference_id, expires_at, is_active) VALUES (?, ?, ?, ?, 1)', [groupId, timerType, instanceId, (0, moment_1.default)().add(Number(secs) || 60, 'seconds').toDate()]);
    }
    catch (err) {
 ...
 */
async function ensureCCParticipantSessions(groupId) {
    try {
        const participantIds = await getGroupParticipantIds(groupId);
        if (participantIds.length === 0)
            return;
        const [existingRows] = await (0, db_1.query)('SELECT participant_id FROM participant_sessions WHERE group_id = ?', [groupId]);
        const existing = new Set((existingRows || []).map((r) => Number(r.participant_id)));
        ...
        for (const id of missing) {
            await (0, db_1.query)(`INSERT INTO participant_sessions (group_id, participant_id, is_online)
                 SELECT ?, ?, 0 FROM DUAL
                 WHERE NOT EXISTS (
 ...
 */
async function finalizeRound1(instanceId, groupId) {
    const data = await getInstanceById(instanceId);
    if (!data)
        return;
    const { template } = data;
    const [claimHeader] = await (0, db_1.query)(`UPDATE cc_game_instances SET status = 'round2', round1_ended_at = NOW(), round2_started_at = NOW(), updated_at = NOW() WHERE id = ? AND status = 'round1'`, [instanceId]);
    if (Number(claimHeader?.affectedRows || 0) === 0)
        return;
    const results = await calculateRound1Results(instanceId, template.round1_top_ingredients);
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round1_complete', { top_ingredients: results });
    ...
    await (0, db_1.query)(`UPDATE cc_game_instances SET round2_turn_index = 0, round2_turn_started_at = NOW(), updated_at = NOW() WHERE id = ?`, [instanceId]);
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round2_turn_changed', { turn_index: 0 });
    await ensureCCTimer(groupId, instanceId, 'cc_round2_turn', template.round2_submit_timer_secs);
}
/**
 ...
 * timer safety net.
 */
async function checkRound1Completion(instanceId, groupId) {
    const participantIds = await getGroupParticipantIds(groupId);
    if (participantIds.length === 0)
        return;
    const [voterRows] = await (0, db_1.query)('SELECT DISTINCT participant_id FROM cc_round1_votes WHERE instance_id = ?', [instanceId]);
    const voted = new Set((voterRows || []).map((r) => Number(r.participant_id)));
 ...
 */
async function advanceRound2ToReview(instanceId, groupId) {
    const data = await getInstanceById(instanceId);
    if (!data)
        return;
    const { template } = data;
    ...
    const [claimHeader] = await (0, db_1.query)(`UPDATE cc_game_instances SET round2_phase = 'review', round2_review_started_at = NOW(), updated_at = NOW()
         WHERE id = ? AND status = 'round2' AND round2_phase = 'submit'`, [instanceId]);
    if (Number(claimHeader?.affectedRows || 0) === 0)
        return;
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round2_review_started', {});
 ...
 */
async function advanceRound2Turn(instanceId, groupId) {
    const data = await getInstanceById(instanceId);
    if (!data)
        return;
    const { instance, template } = data;
    ...
    const [claimHeader] = await (0, db_1.query)(`UPDATE cc_game_instances SET round2_turn_index = ?, round2_turn_started_at = NOW(), updated_at = NOW()
         WHERE id = ? AND status = 'round2' AND round2_phase = 'submit' AND round2_turn_index = ?`, [next, instanceId, current]);
    if (Number(claimHeader?.affectedRows || 0) === 0)
        return;
    ...
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round2_turn_changed', { turn_index: next });
    await ensureCCTimer(groupId, instanceId, 'cc_round2_turn', template.round2_submit_timer_secs);
}
 ...
 */
async function finalizeRound2Review(instanceId, groupId) {
    const [stepRows] = await (0, db_1.query)(`SELECT s.id,
         ...
         FROM cc_round2_steps s WHERE s.instance_id = ? AND s.status = 'submitted'`, [instanceId]);
    if (!stepRows || stepRows.length === 0)
        return; // already resolved by a concurrent caller
    for (const s of stepRows) {
        const keep = Number(s.keep_votes);
        const remove = Number(s.remove_votes);
        const status = remove > keep ? 'removed' : 'kept'; // ties -> kept
        await (0, db_1.query)('UPDATE cc_round2_steps SET status = ? WHERE id = ?', [status, s.id]);
    }
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round2_review_complete', {});
 ...
 */
async function checkRound2ReviewCompletion(instanceId, groupId) {
    const participantIds = await getGroupParticipantIds(groupId);
    if (participantIds.length === 0)
        return;
    const [stepRows] = await (0, db_1.query)(`SELECT id FROM cc_round2_steps WHERE instance_id = ? AND status = 'submitted'`, [instanceId]);
    if (!stepRows || stepRows.length === 0)
        return;
    const [voteRows] = await (0, db_1.query)(`SELECT step_id, COUNT(*) as c FROM cc_round2_step_votes WHERE instance_id = ? GROUP BY step_id`, [instanceId]);
    const votesByStep = new Map((voteRows || []).map((r) => [Number(r.step_id), Number(r.c)]));
    const allDone = stepRows.every((s) => (votesByStep.get(Number(s.id)) || 0) >= participantIds.length);
    if (allDone) {
 ...
 */
async function startRound3DiscussionTimer(groupId, instanceId) {
    const data = await getInstanceById(instanceId);
    if (!data)
        return;
 ...
 */
async function advanceRound3ToVoting(instanceId, groupId) {
    const data = await getInstanceById(instanceId);
    if (!data)
        return;
    const { template, instance } = data;
    const [claimHeader] = await (0, db_1.query)(`UPDATE cc_game_instances SET status = 'round3_voting', round3_discussion_ended_at = NOW(), round3_voting_started_at = NOW(), updated_at = NOW() WHERE id = ? AND status = 'round3_discussion'`, [instanceId]);
    if (Number(claimHeader?.affectedRows || 0) === 0)
        return;
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round3_voting_started', {});
 ...
 */
async function offerDoubleDown(instanceId, groupId, impostorId) {
    try {
        const participantIds = await getGroupParticipantIds(groupId);
        ...
        await (0, db_1.query)(`UPDATE cc_game_instances SET double_down_participant_id = ?, double_down_status = 'offered' WHERE id = ?`, [chosen, instanceId]);
        const [sessionRows] = await (0, db_1.query)('SELECT socket_id FROM participant_sessions WHERE group_id = ? AND participant_id = ? LIMIT 1', [groupId, chosen]);
        const socketId = sessionRows?.[0]?.socket_id;
        if (socketId) {
            server_1.io.to(socketId).emit('cc_double_down_offer', {});
        }
    }
    catch (err) {
 ...
 */
async function respondToDoubleDown(instanceId, participantId, accept) {
    await (0, db_1.query)(`UPDATE cc_game_instances SET double_down_status = ?
         WHERE id = ? AND double_down_participant_id = ? AND double_down_status = 'offered'`, [accept ? 'accepted' : 'declined', instanceId, participantId]);
}
/**
 ...
 */
async function finalizeRound3(instanceId, groupId) {
    const [claimHeader] = await (0, db_1.query)(`UPDATE cc_game_instances SET status = 'completed', round3_voting_ended_at = NOW(), finished_at = NOW(), updated_at = NOW() WHERE id = ? AND status = 'round3_voting'`, [instanceId]);
    if (Number(claimHeader?.affectedRows || 0) === 0)
        return;
    const [instRows] = await (0, db_1.query)(`SELECT impostor_participant_id, double_down_participant_id, double_down_status FROM cc_game_instances WHERE id = ?`, [instanceId]);
    ...
    const [voteRows] = await (0, db_1.query)(`SELECT voted_for_participant_id,
                SUM(CASE WHEN participant_id = ? AND ? THEN 2 ELSE 1 END) AS vote_count
         FROM cc_round3_impostor_votes WHERE instance_id = ?
         GROUP BY voted_for_participant_id ORDER BY vote_count DESC LIMIT 1`, [doubleDownParticipantId, doubleDownAccepted, instanceId]);
    ...
    await (0, db_1.query)(`UPDATE cc_game_instances SET group_won = ? WHERE id = ?`, [groupWon ? 1 : 0, instanceId]);
    ...
    if (doubleDownAccepted && doubleDownParticipantId != null) {
        const [ddVoteRows] = await (0, db_1.query)(`SELECT voted_for_participant_id FROM cc_round3_impostor_votes WHERE instance_id = ? AND participant_id = ? LIMIT 1`, [instanceId, doubleDownParticipantId]);
        ...
        if (ddWasWrong) {
            const [sessionRows] = await (0, db_1.query)('SELECT id FROM participant_sessions WHERE group_id = ? AND participant_id = ? LIMIT 1', [groupId, doubleDownParticipantId]);
            const sessionId = sessionRows?.[0]?.id;
            if (sessionId) {
                await (0, db_1.query)('UPDATE participant_sessions SET total_score = total_score - 50 WHERE id = ?', [sessionId]);
                await (0, db_1.query)(`INSERT INTO score_logs (participant_session_id, points, reason, created_at, updated_at)
                        VALUES (?, -50, 'cc_double_down_wrong', NOW(), NOW())`, [sessionId]);
    ...
    await (0, db_1.query)(`UPDATE game_groups SET status = 'completed', completed_at = ?, retention_purge_at = ? WHERE id = ? AND status NOT IN ('completed', 'incomplete')`, [completedAt, retentionPurgeAt, groupId]);
    server_1.io.to(`cc-instance-${instanceId}`).emit('cc_round3_complete', {
 ...
 */
async function checkRound3VotingCompletion(instanceId, groupId) {
    const participantIds = await getGroupParticipantIds(groupId);
    if (participantIds.length === 0)
        return;
    const [voterRows] = await (0, db_1.query)('SELECT DISTINCT participant_id FROM cc_round3_impostor_votes WHERE instance_id = ?', [instanceId]);
    const voted = new Set((voterRows || []).map((r) => Number(r.participant_id)));
    if (participantIds.every((id) => voted.has(id))) {
 ...
 */
async function getCCRules(templateId) {
    const [rows] = await (0, db_1.query)(`SELECT id, rule_text, \`order\` FROM cc_game_rules WHERE template_id = ? ORDER BY \`order\` ASC`, [templateId]);
    return (rows || []).map((r) => ({ id: Number(r.id), rule_text: r.rule_text, order: Number(r.order) }));
}
/**
 ...
 */
async function getOtherDishes(groupId, participantId) {
    const [groupRows] = await (0, db_1.query)('SELECT booking_id FROM game_groups WHERE id = ?', [groupId]);
    if (!groupRows || groupRows.length === 0)
        return [];
    const bookingId = groupRows[0].booking_id;
    const [rows] = await (0, db_1.query)(`SELECT gg.id as group_id, gg.group_name, ci.id AS instance_id, ci.dish_name
         FROM game_groups gg
         ...
         ORDER BY RAND() LIMIT 3`, [bookingId, groupId, participantId]);
    if (!rows || rows.length === 0)
        return [];
    const groupIds = rows.map((r) => Number(r.group_id));
    const [countRows] = await (0, db_1.query)(`SELECT r.rated_group_id, rc.slug, COUNT(*) as c
         FROM cc_ratings r JOIN cc_rating_categories rc ON rc.id = r.category_id
         WHERE r.rated_group_id IN (?) GROUP BY r.rated_group_id, rc.slug`, [groupIds]);
    const countsByGroup = new Map();
    for (const c of countRows || []) {
        const gid = Number(c.rated_group_id);
        if (!countsByGroup.has(gid))
            countsByGroup.set(gid, {});
    ...
    const instanceIds = rows.map((r) => Number(r.instance_id));
    const [stepRows] = await (0, db_1.query)(`SELECT instance_id, step_letter, step_text FROM cc_round2_steps
         WHERE instance_id IN (?) AND status != 'removed'
         ORDER BY instance_id, step_letter ASC`, [instanceIds]);
    const stepsByInstance = new Map();
    for (const s of stepRows || []) {
        const iid = Number(s.instance_id);
        if (!stepsByInstance.has(iid))
            stepsByInstance.set(iid, []);
 ...
 */
async function submitRating(instanceId, participantId, ratedGroupId, categoryId) {
    await (0, db_1.query)(`INSERT INTO cc_ratings (instance_id, category_id, voting_participant_id, rated_group_id, created_at)
         VALUES (?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE created_at = created_at`, [instanceId, categoryId, participantId, ratedGroupId]);
}
/**
 ...
 */
async function getAwards(groupId) {
    const [groupRows] = await (0, db_1.query)('SELECT booking_id FROM game_groups WHERE id = ?', [groupId]);
    if (!groupRows || groupRows.length === 0)
        return null;
    const bookingId = groupRows[0].booking_id;
    const [groupsRows] = await (0, db_1.query)(`SELECT gg.id as group_id, gg.group_name, ci.dish_name, ci.finished_at
         FROM game_groups gg JOIN cc_game_instances ci ON ci.group_id = gg.id
         WHERE gg.booking_id = ? AND ci.status = 'completed' AND ci.dish_name IS NOT NULL`, [bookingId]);
    const categories = await getCCRatingCategories();
    const [tallyRows] = await (0, db_1.query)(`SELECT r.category_id, r.rated_group_id, COUNT(*) as c
         FROM cc_ratings r
         JOIN game_groups gg ON gg.id = r.rated_group_id
         WHERE gg.booking_id = ?
         GROUP BY r.category_id, r.rated_group_id`, [bookingId]);
    const finishedAtByGroup = new Map();
    ...
    for (const t of tallyRows || []) {
        ...
        if (!byCategory.has(catId))
            byCategory.set(catId, new Map());
        byCategory.get(catId).set(Number(t.rated_group_id), Number(t.c));
    }
    const awardWinnerByCategory = new Map();
    for (const cat of categories) {
        ...
        if (bestGroupId != null)
            awardWinnerByCategory.set(cat.id, bestGroupId);
    }
    ...
    const [myInstanceRows] = await (0, db_1.query)(`SELECT id, impostor_participant_id, group_won, dish_name, double_down_participant_id, double_down_status
         FROM cc_game_instances WHERE group_id = ?`, [groupId]);
    ...
    if (myInstance) {
        ...
        const [voteRows] = await (0, db_1.query)(`SELECT voted_for_participant_id,
                    SUM(CASE WHEN participant_id = ? AND ? THEN 2 ELSE 1 END) AS vote_count
             FROM cc_round3_impostor_votes WHERE instance_id = ?
             GROUP BY voted_for_participant_id ORDER BY vote_count DESC LIMIT 1`, [doubleDownParticipantId, doubleDownAccepted, myInstance.id]);
        mostVotedId = voteRows.length > 0 ? Number(voteRows[0].voted_for_participant_id) : null;
        if (doubleDownAccepted && doubleDownParticipantId != null) {
            const [ddVoteRows] = await (0, db_1.query)(`SELECT voted_for_participant_id FROM cc_round3_impostor_votes WHERE instance_id = ? AND participant_id = ? LIMIT 1`, [myInstance.id, doubleDownParticipantId]);
            const ddTarget = ddVoteRows?.[0]?.voted_for_participant_id ? Number(ddVoteRows[0].voted_for_participant_id) : null;
    ...
    if (myInstance) {
        const [reactionRows] = await (0, db_1.query)(`SELECT rc.slug, COUNT(*) AS c
             FROM cc_ratings r JOIN cc_rating_categories rc ON rc.id = r.category_id
             WHERE r.rated_group_id = ? GROUP BY rc.slug`, [groupId]);
        for (const row of reactionRows || [])
            reactionCounts[row.slug] = Number(row.c);
    }
    return {
        groups,
        my_group: {
