...
exports.reopenCaseSummary = exports.useWitnessPasscard = exports.endLieDetectorRound = exports.getLieDetectorTally = exports.voteLieDetector = exports.startLieDetector = exports.submitAccusation = exports.answerQuestion = exports.askQuestion = exports.getGameState = void 0;
...
async function getSessionForParticipant(groupId, participantId) {
    const [rows] = await (0, db_1.query)(`SELECT ps.*, gr.role_type, gr.character_name FROM participant_sessions ps
 ...
 */
async function emitScoresUpdate(groupId) {
    try {
        const [rows] = await (0, db_1.query)('SELECT id AS session_id, total_score FROM participant_sessions WHERE group_id = ?', [groupId]);
...
async function getLieDetectorTallyData(roundId) {
    const [rows] = await (0, db_1.query)(`SELECT vote_value, COUNT(*) as cnt FROM votes WHERE reference_id = ? AND reference_type = 'lie_detector' GROUP BY vote_value`, [roundId]);
    const tally = { believable: 0, suspicious: 0 };
    for (const r of rows) {
        if (r.vote_value === 'believable')
            tally.believable = Number(r.cnt);
        if (r.vote_value === 'suspicious')
            tally.suspicious = Number(r.cnt);
    }
    return tally;
}
/**
 ...
 */
exports.getGameState = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const participantId = req.query.participant_id;
    const { group_id } = req.params;
    if (!participantId)
        throw new AppError_1.AppError('participant_id is required', 400);
    const [groups] = await (0, db_1.query)('SELECT * FROM game_groups WHERE id = ? LIMIT 1', [group_id]);
    const group = groups?.[0];
    if (!group)
        throw new AppError_1.AppError('Group not found', 404);
    const [sessions] = await (0, db_1.query)(`SELECT ps.*, p.name as participant_name, p.email as participant_email,
    ...
    const userSession = sessions.find((s) => String(s.participant_id) === String(participantId));
    if (!userSession)
    ...
    const [witnessPasscards] = await (0, db_1.query)('SELECT * FROM witness_passcards WHERE group_id = ?', [group_id]);
    const [results] = await (0, db_1.query)('SELECT * FROM results WHERE group_id = ? ORDER BY id DESC', [group_id]);
    const [accusations] = await (0, db_1.query)('SELECT participant_session_id FROM group_accusations WHERE group_id = ?', [group_id]);
    const myAccusationSubmitted = accusations.some((a) => String(a.participant_session_id) === String(userSession.id));
    const groupPayload = {
        ...group,
        participant_sessions: sessions.map((s) => ({
            ...s,
    ...
    return (0, apiResponse_1.successResponse)(res, 'Game state retrieved', {
        group: (0, serializer_1.serializeData)(groupPayload),
        my_role: (0, serializer_1.serializeData)(groupPayload.participant_sessions.find((s) => String(s.id) === String(userSession.id))?.game_roles),
 ...
 */
exports.askQuestion = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!investigatorSession || investigatorSession.role_type !== 'investigator') {
        throw new AppError_1.AppError('Only the Investigator can ask questions', 403);
    }
    ...
    const [roundRows] = await (0, db_1.query)(`SELECT * FROM lie_detector_rounds WHERE group_id = ? AND status = 'active' ORDER BY id DESC LIMIT 1`, [group_id]);
    const activeLieRound = roundRows?.[0];
    if (activeLieRound) {
        const lieMax = Number(config?.lie_detector_max_questions ?? 3);
        const [lieCountRows] = await (0, db_1.query)(`SELECT COUNT(*) as cnt FROM questions WHERE group_id = ? AND created_at >= ?`, [group_id, activeLieRound.created_at]);
        if (Number(lieCountRows?.[0]?.cnt || 0) >= lieMax) {
            throw new AppError_1.AppError(`Maximum of ${lieMax} questions reached in Lie Detector mode`, 400);
        }
    }
    else {
        const [countRows] = await (0, db_1.query)(`SELECT COUNT(*) as cnt FROM questions q
              WHERE q.group_id = ?
                AND NOT EXISTS (
        ...
        if (Number(countRows?.[0]?.cnt || 0) >= maxQuestions) {
            throw new AppError_1.AppError(`Maximum of ${maxQuestions} questions reached`, 400);
        }
    }
    const question = await (0, db_1.withTransaction)(async (conn) => {
        ...
        await conn.query(`INSERT INTO timers (group_id, timer_type, reference_id, expires_at, is_active, created_at, updated_at)
                VALUES (?, 'question_response', ?, ?, 1, NOW(), NOW())`, [group_id, questionId, (0, moment_1.default)().add(responseSecs, 'seconds').toDate()]);
        const [rows] = await conn.query(`SELECT * FROM questions WHERE id = ? LIMIT 1`, [questionId]);
        return rows?.[0];
    });
    server_1.io.to(`group_${group_id}`).emit('new_question', (0, serializer_1.serializeData)(question));
    await emitScoresUpdate(group_id);
    return (0, apiResponse_1.successResponse)(res, 'Question asked successfully', (0, serializer_1.serializeData)(question));
});
/**
 ...
 */
exports.answerQuestion = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { question_id, participant_id, answer_text } = req.body;
    const [questions] = await (0, db_1.query)('SELECT * FROM questions WHERE id = ? LIMIT 1', [question_id]);
    const question = questions?.[0];
    if (!question)
        throw new AppError_1.AppError('Question not found', 404);
    ...
    if (!participantSession || participantSession.id !== question.asked_to) {
        throw new AppError_1.AppError('You are not the one asked this question', 403);
    }
    const [existingAnswers] = await (0, db_1.query)('SELECT id FROM answers WHERE question_id = ? LIMIT 1', [question_id]);
    if (existingAnswers?.[0])
        throw new AppError_1.AppError('Already answered', 400);
    ...
    const lateBySecs = now.diff((0, moment_1.default)(question.created_at), 'seconds');
    ...
    const answer = await (0, db_1.withTransaction)(async (conn) => {
        ...
        if (lieDetectorBonus > 0) {
            ...
            await conn.query(`INSERT INTO score_logs (participant_session_id, points, reason, created_at, updated_at)
                    VALUES (?, ?, 'lie_detector_answer', NOW(), NOW())`, [participantSession.id, lieDetectorBonus]);
        }
        // The answer arrived — defuse the timeout timer so it doesn't also fire.
        await conn.query(`UPDATE timers SET is_active = 0 WHERE group_id = ? AND timer_type = 'question_response' AND reference_id = ?`, [question.group_id, question_id]);
        const [rows] = await conn.query(`SELECT * FROM answers WHERE question_id = ? ORDER BY id DESC LIMIT 1`, [
            question_id,
        ]);
        return rows?.[0];
    });
    server_1.io.to(`group_${question.group_id}`).emit('new_answer', (0, serializer_1.serializeData)(answer));
    await emitScoresUpdate(question.group_id);
    return (0, apiResponse_1.successResponse)(res, 'Answered successfully', (0, serializer_1.serializeData)(answer));
});
/**
 ...
 */
exports.submitAccusation = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const result = await (0, verdictScoringService_1.submitAccusation)(group_id, session.id, accused_session_id, reasoning);
    return (0, apiResponse_1.successResponse)(res, 'Accusation submitted', result);
});
/**
 * Start Lie Detector Round
 * Investigator-only, once per game.
 */
exports.startLieDetector = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { group_id, participant_id, suspect_session_id } = req.body;
    const investigatorSession = await getSessionForParticipant(group_id, participant_id);
    if (!investigatorSession || investigatorSession.role_type !== 'investigator') {
        throw new AppError_1.AppError('Only the Investigator can start the lie detector', 403);
    }
    const [usedRows] = await (0, db_1.query)(`SELECT id FROM score_logs WHERE participant_session_id = ? AND reason = 'lie_detector_used' LIMIT 1`, [investigatorSession.id]);
    if (usedRows?.[0])
    ...
    server_1.io.to(`group_${group_id}`).emit('lie_detector_started', (0, serializer_1.serializeData)(round));
    await emitScoresUpdate(group_id);
    return (0, apiResponse_1.successResponse)(res, 'Lie detector round started', (0, serializer_1.serializeData)(round));
});
/**
...
exports.voteLieDetector = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        ...
        if (err?.code === 'ER_DUP_ENTRY') {
            throw new AppError_1.AppError('You have already voted on this answer', 400);
        }
        throw err;
    }
...
exports.getLieDetectorTally = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const tally = await getLieDetectorTallyData(round_id);
    return (0, apiResponse_1.successResponse)(res, 'Tally retrieved', tally);
});
/**
 ...
 */
exports.endLieDetectorRound = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { group_id, participant_id, round_id } = req.body;
    const investigatorSession = await getSessionForParticipant(group_id, participant_id);
    if (!investigatorSession || investigatorSession.role_type !== 'investigator') {
        throw new AppError_1.AppError('Only the Investigator can end the lie detector round', 403);
    }
    await (0, db_1.query)(`UPDATE lie_detector_rounds SET status = 'completed', updated_at = NOW() WHERE id = ?`, [round_id]);
    await (0, db_1.query)(`UPDATE timers SET is_active = 0 WHERE group_id = ? AND timer_type = 'lie_detector'`, [group_id]);
    server_1.io.to(`group_${group_id}`).emit('phase_changed', {
        new_phase: 'questioning',
        message: 'Lie Detector round has ended.',
    });
    return (0, apiResponse_1.successResponse)(res, 'Lie detector round ended');
});
/**
 * Use Witness Passcard
 */
exports.useWitnessPasscard = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!witnessSession || witnessSession.role_type !== 'witness') {
        throw new AppError_1.AppError('Only the Witness can use the passcard', 403);
    }
    const [existingPasscards] = await (0, db_1.query)('SELECT * FROM witness_passcards WHERE group_id = ? AND participant_session_id = ? LIMIT 1', [group_id, witnessSession.id]);
    ...
    await emitScoresUpdate(group_id);
    const [investigatorSessions] = await (0, db_1.query)(`SELECT ps.* FROM participant_sessions ps
            LEFT JOIN game_roles gr ON gr.id = ps.role_id
    ...
    if (investigatorSession && investigatorSession.socket_id) {
        server_1.io.to(investigatorSession.socket_id).emit('witness_passcard_used', {
            message: 'The Witness has used their secret passcard!',
        });
    }
    return (0, apiResponse_1.successResponse)(res, 'Passcard used successfully', (0, serializer_1.serializeData)(passcard));
});
/**
 * Reopen Case Summary — Investigator-only, once per game.
 */
exports.reopenCaseSummary = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { group_id, participant_id } = req.body;
    const investigatorSession = await getSessionForParticipant(group_id, participant_id);
    if (!investigatorSession || investigatorSession.role_type !== 'investigator') {
        throw new AppError_1.AppError('Only the Investigator can reopen the case summary', 403);
    }
    const [groups] = await (0, db_1.query)('SELECT * FROM game_groups WHERE id = ? LIMIT 1', [group_id]);
    if (!groups?.[0])
        throw new AppError_1.AppError('Group not found', 404);
    const [reopenLogs] = await (0, db_1.query)(`SELECT id FROM score_logs WHERE participant_session_id = ? AND reason = 'reopen_case_summary' LIMIT 1`, [investigatorSession.id]);
    if (reopenLogs?.[0])
        throw new AppError_1.AppError('You can only reopen the case summary once', 400);
    await (0, db_1.withTransaction)(async (conn) => {
        await conn.query(`INSERT INTO timers (group_id, timer_type, expires_at, is_active, created_at, updated_at)
                VALUES (?, 'case_summary', ?, 1, NOW(), NOW())`, [group_id, (0, moment_1.default)().add(5, 'minutes').toDate()]);
        await conn.query(`INSERT INTO score_logs (participant_session_id, points, reason, created_at, updated_at)
                VALUES (?, 0, 'reopen_case_summary', NOW(), NOW())`, [investigatorSession.id]);
    });
    server_1.io.to(`group_${group_id}`).emit('case_summary_reopened', {
        message: 'Investigator has reopened the case summary for 5 more minutes!',
    });
    return (0, apiResponse_1.successResponse)(res, 'Case summary reopened successfully');
});
