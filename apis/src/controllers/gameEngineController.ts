...
export const getGameState = asyncHandler(async (req: Request, res: Response) => {
    ...
    const [gameCfgRows] = await query<any>(
        `SELECT a.game_duration_secs, a.case_summary_view_secs
         FROM game_groups gg
         JOIN organizer_bookings ob ON ob.id = gg.booking_id
         JOIN activities a ON a.id = ob.activity_id
         WHERE gg.id = ? LIMIT 1`,
    ...
    const caseTimer = (timers as any[]).find(
        (t: any) => t.timer_type === 'case_summary' && Number(t.is_active) === 1
    );
    const questioningTimer = (timers as any[]).find(
        (t: any) => t.timer_type === 'questioning' && Number(t.is_active) === 1
    );
    ...
    const [lieVoteRows] = await query<any>(
        `SELECT reference_id AS question_id, vote_value, COUNT(*) AS cnt
            FROM votes WHERE group_id = ? AND reference_type = 'lie_detector'
            GROUP BY reference_id, vote_value`,
        [group_id]
    );
    const lieVoteTallies: Record<string, { believable: number; suspicious: number }> = {};
    for (const r of lieVoteRows as any[]) {
        const key = String(r.question_id);
        if (!lieVoteTallies[key]) lieVoteTallies[key] = { believable: 0, suspicious: 0 };
        if (r.vote_value === 'believable') lieVoteTallies[key].believable = Number(r.cnt);
    ...
    const question = await withTransaction(async (conn) => {
        ...
        const [insertResult] = await conn.query<any>(
            `INSERT INTO questions (group_id, asked_by, asked_to, question_text, points_awarded, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
            [group_id, investigatorSession.id, asked_to_session_id, question_text, questionBonus]
        );
...
export const startLieDetector = asyncHandler(async (req: Request, res: Response) => {
    ...
    const round = await withTransaction(async (conn) => {
        ...
        await conn.query(
            `INSERT INTO score_logs (participant_session_id, points, reason, created_at, updated_at)
                VALUES (?, ?, 'lie_detector_used', NOW(), NOW())`,
            [investigatorSession.id, initBonus]
        );
...
export const voteLieDetector = asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
        await query(
            ...
            [group_id, session.id, question_id, vote_value]
        );
    } catch (err: any) {
    ...
    if (session.role_type && session.role_type !== 'investigator') {
        ...
        const [roundRows] = await query<any>(
            `SELECT created_at FROM lie_detector_rounds WHERE id = ? LIMIT 1`,
            [round_id]
        );
        const roundStart = roundRows?.[0]?.created_at ?? null;
        const [alreadyScored] = await query<any>(
            `SELECT id FROM score_logs
                WHERE participant_session_id = ? AND reason = 'lie_detector_vote' AND created_at >= ? LIMIT 1`,
            [session.id, roundStart]
        );
        if ((alreadyScored as any[]).length === 0) {
            if (voteBonus !== 0) {
                await query(`UPDATE participant_sessions SET total_score = total_score + ? WHERE id = ?`, [voteBonus, session.id]);
            }
            await query(
                `INSERT INTO score_logs (participant_session_id, points, reason, created_at, updated_at)
                    VALUES (?, ?, 'lie_detector_vote', NOW(), NOW())`,
                [session.id, voteBonus]
            );
            await emitScoresUpdate(group_id);
        }
    }
    ...
    io.to(`group_${group_id}`).emit('new_vote', { round_id, question_id, tally });
...
export const useWitnessPasscard = asyncHandler(async (req: Request, res: Response) => {
    ...
    const passcard = await withTransaction(async (conn) => {
        ...
        await conn.query(
            `INSERT INTO score_logs (participant_session_id, points, reason, created_at, updated_at)
                VALUES (?, ?, 'witness_passcard_used', NOW(), NOW())`,
            [witnessSession.id, passcardBonus]
        );
